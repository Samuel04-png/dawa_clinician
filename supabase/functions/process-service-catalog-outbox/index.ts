...
export async function handleRequest(req: Request): Promise<Response> {
  ...
  const callbackUrl = Deno.env.get('DAWA_MOM_SERVICE_SYNC_URL') ??
    'https://himbfndvsuwiudtzjojh.supabase.co/functions/v1/receive-service-catalog-sync';
  const callbackSecret = Deno.env.get('DAWA_MOM_SYNC_SECRET');
  if (!supabaseUrl || !serviceRoleKey || !callbackSecret) {
    return jsonResponse({ error: 'Service catalog worker is not configured.' }, 500);
  }
  ...
  const workerId = `dawa-clinician-service-${crypto.randomUUID()}`;
  const supabase = createClient(supabaseUrl, serviceRoleKey, {
    auth: { autoRefreshToken: false, persistSession: false },
  });

  const { data, error } = await supabase.rpc('claim_service_catalog_sync_jobs', {
    p_limit: limit,
    p_worker_id: workerId,
  });
  if (error) {
    console.error('[process-service-catalog-outbox] Could not claim jobs.');
    return jsonResponse({ error: 'Service sync jobs could not be claimed.' }, 500);
  }
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      success = upstream.ok;
      errorCode = success ? null : (text(upstreamPayload.code) ?? `HTTP_${upstream.status}`).slice(0, 120);
      retryable = upstreamPayload.retryable === true ||
        upstream.status === 408 ||
        upstream.status === 429 ||
        upstream.status >= 500;
    } catch {
    ...
    const { error: completeError } = await supabase.rpc('complete_service_catalog_sync_job', {
      p_job_id: job.id,
      p_worker_id: workerId,
      p_success: success,
    ...
    if (completeError) {
      console.error('[process-service-catalog-outbox] Could not complete a job.');
      retrying += 1;
    } else if (success) {
      completed += 1;
    } else if (permanent) {
